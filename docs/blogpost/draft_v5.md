...
- **Page Index** - Implemented by some Chinese developers recently. The core idea: use file system + progressive disclosure (see [Anthropic's Skills Documentation - Progressive Disclosure]). You can use prompts to let the memory system restructure itself - extremely powerful. I implemented this in version 1: slower and had some minor issues, but more powerful than vector database in some contexts. Switched to vector DB because it's lighter, faster, and sometimes even better for the main use case.
- Knowledge Graph
- Hinsight article on memory
...
| **What** | Episodic/Semantic/Procedural | Extract both episodic+semantic simultaneously |
| **When (Store)** | Various approaches | After task completes → check duplicates → merge or store new |
...
**Written:** 2026-01-13
**Status:** DRAFT V5 - Moved duplicate checking to "When to Store" section
